...
class PointCloudCenterAndNormalize(object):
    def __init__(
        self, centering=True, normalize=True, gravity_dim=2, append_xyz=False, **kwargs
    ):
    ...
    def __call__(self, data):
        if hasattr(data, "keys"):
            ...
            if self.centering:
                data["pos"] = data["pos"] - torch.mean(
                    data["pos"], axis=0, keepdims=True
                )

            if self.normalize:
                m = torch.max(
                    torch.sqrt(torch.sum(data["pos"] ** 2, axis=-1, keepdims=True)),
                    axis=0,
                    keepdims=True,
                )[0]
                data["pos"] = data["pos"] / m
            ...
            if self.normalize:
                m = torch.max(
                    torch.sqrt(torch.sum(data**2, axis=-1, keepdims=True)),
                    axis=0,
                    keepdims=True,
                )[0]
                data = data / m
...
class PointCloudXYZAlign(object):
    ...
    def __init__(self, gravity_dim=2, **kwargs):
        self.gravity_dim = gravity_dim
...
class PointCloudScaling(object):
    ...
    def __call__(self, data):
        ...
        scale = (
            torch.rand(3 if self.anisotropic else 1, dtype=torch.float32, device=device)
        ...
        if self.use_mirroring:
            ...
            mirror = (torch.rand(3, device=device) > self.mirror).to(
                torch.float32
            ) * 2 - 1
            scale *= mirror
...
class PointCloudTranslation(object):
    def __init__(self, shift=[0.2, 0.2, 0.0], **kwargs):
        self.shift = torch.from_numpy(np.array(shift)).to(torch.float32)

    def __call__(self, data):
        device = data["pos"].device if hasattr(data, "keys") else data.device
        translation = torch.rand(3, dtype=torch.float32, device=device) * self.shift.to(
            device
        )
        if hasattr(data, "keys"):
    ...
    def __call__(self, data):
        ...
        for i, s in enumerate(self.scale_xyz):
            if not s:
                scale[i] = 1
        translation = (
            (torch.rand(3, dtype=torch.float32, device=device) - 0.5)
            * 2
    ...
    def __call__(self, data):
        ...
        for i, s in enumerate(self.scale_xyz):
            if not s:
                scale[i] = 1
        if hasattr(data, "keys"):
            noise = (torch.randn_like(data["pos"]) * self.noise_std).clamp_(
                -self.noise_clip, self.noise_clip
            )
            data["pos"] = torch.mul(data["pos"], scale) + noise
        else:
            noise = (torch.randn_like(data) * self.noise_std).clamp_(
                -self.noise_clip, self.noise_clip
            )
    ...
    def __call__(self, data):
        ...
        if isinstance(self.angle, collections.abc.Iterable):
            ...
            rot_mat = torch.tensor(
                rot_mats[0] @ rot_mats[1] @ rot_mats[2],
                dtype=torch.float32,
                device=device,
            )
        else:
...
class ChromaticPerDropGPU(object):
    ...
    def __call__(self, data):
        colors_drop = (torch.rand((data["x"].shape[0], 1)) > self.color_drop).to(
            torch.float32
        )
...
class ChromaticNormalize(object):
    ...
    def __call__(self, data):
        ...
        if data["x"][:, :3].max() > 1:
            data["x"][:, :3] /= 255.0
        data["x"][:, :3] = (
            data["x"][:, :3] - self.color_mean.to(device)
        ) / self.color_std.to(device)
        return data
...
class Cutmix:
    ...
    def __init__(
        self, cutmix_alpha=0.3, prob=1.0, label_smoothing=0.1, num_classes=1000
    ):
    ...
    def __call__(self, data, target):
        ...
        target = mixup_target(
            target, self.num_classes, lam, self.label_smoothing, device
        )
        return data, target
