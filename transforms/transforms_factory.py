...
def build_transforms_from_cfg(split, datatransforms_cfg):
    ...
    if len(transform_list) > 1:
        for t in transform_list:
            point_transforms.append(
                DataTransforms.build({"NAME": t}, default_args=transform_args)
            )
        return compose_fn(point_transforms)
    else:
        return DataTransforms.build(
            {"NAME": transform_list[0]}, default_args=transform_args
        )
