...
def partnet_metrics(num_classes, num_parts, objects, preds, targets):
    ...
    part_intersect = [
        np.zeros((num_parts[o_l]), dtype=np.float32) for o_l in range(num_classes)
    ]
    part_union = [
        np.zeros((num_parts[o_l]), dtype=np.float32) + 1e-6
        for o_l in range(num_classes)
    ]
    ...
    part_iou = [
        np.divide(part_intersect[o_l][1:], part_union[o_l][1:])
        for o_l in range(num_classes)
    ]
