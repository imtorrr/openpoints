...
def preprocess_data(num_nodes, edges, node_feats, edge_feats):
    node_feats = node_feats + np.arange(
        1,
        node_feats.shape[-1] * NODE_FEATURES_OFFSET + 1,
        NODE_FEATURES_OFFSET,
        dtype=np.int16,
    )
    edge_feats = edge_feats + np.arange(
        1,
        edge_feats.shape[-1] * EDGE_FEATURES_OFFSET + 1,
        EDGE_FEATURES_OFFSET,
        dtype=np.int16,
    )
...
class StructuralDataset(GraphDataset):
    ...
    def __getitem__(self, index):
        ...
        node_feats, dist_mat, edge_feats_mat = preprocess_data(
            num_nodes, edges, node_feats, edge_feats
        )
