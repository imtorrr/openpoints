...
def load_data_partseg(partition, DATA_DIR):
    ...
    if partition == "trainval":
        file = glob.glob(os.path.join(DATA_DIR, "*train*.h5")) + glob.glob(
...
class ShapeNetPart(Dataset):
    ...
    def __getitem__(self, item):
        ...
        data = {"pos": pointcloud, "x": feat, "y": seg}
...
class ShapeNetPartNormal(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if presample and not os.path.exists(filename):
            ...
            self.data, self.cls = [], []
            npoints = []
            for cat, filepath in tqdm(
                self.datapath, desc=f"Sample ShapeNetPart {split} split"
            ):
    ...
    def __getitem__(self, index):
        ...
        data = {"pos": point_set[:, 0:3], "x": point_set[:, 3:6], "cls": cls, "y": seg}
...
class ShapeNetPartCurve(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data = {"pos": point_set[:, 0:3], "cls": cls.astype(np.int64), "y": seg}
        if self.transform is not None:
            data = self.transform(data)
