...
class MP40(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not data_list.exists():
            ...
            train_list = str(root_dir / ("mattportobject_train_list.txt"))
            val_list = str(root_dir / ("mattportobject_val_list.txt"))
            test_list = str(root_dir / ("mattportobject_test_list.txt"))
        ...
        if not os.path.exists(self.save_path):
            ...
            for index in tqdm(range(len(self.datapath)), total=len(self.datapath)):
                ...
                if self.uniform:
                    point_set = (
                        fps(torch.from_numpy(point_set).unsqueeze(0).cuda(), npoints)
                        .squeeze(0)
                        .cpu()
                        .numpy()
                    )
        ...
        cls_mapping = pd.read_csv(
            str(pathlib.Path(__file__).parent / "category_mapping.tsv"),
            skiprows=1,
            header=None,
            sep="\t",
            usecols=[0, 16],
    ...
    def __getitem__(self, index):
        ...
        if "heights" in data.keys():
            data["x"] = torch.cat((data["pos"], data["heights"], data["x"]), dim=1)
        else:
