...
class S3DISSphere(data.Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # prepare data
        filename = os.path.join(processed_dir, f"{split}_{voxel_size:.3f}_data.pkl")
        if not os.path.exists(filename):
            ...
            for cloud_idx, cloud_name in enumerate(cloud_names):
                ...
                for i, room_folder in enumerate(room_folders):
                    ...
                    room_npoints = []
                    for object_name in os.listdir(
                        os.path.join(room_folder, "Annotations")
                    ):
                ...
                if os.path.exists(sub_cloud_file):
                    ...
                    with open(sub_cloud_file, "wb") as f:
                        pickle.dump(
                            (sub_points, sub_colors, sub_labels, search_tree), f
                        )
            ...
            with open(filename, "wb") as f:
                pickle.dump(
                    (
                        self.clouds_points,
                        self.clouds_points_colors,
                        self.clouds_points_labels,
                        self.clouds_rooms,
                        self.sub_clouds_points,
                        self.sub_clouds_points_colors,
                        self.sub_clouds_points_labels,
                        self.sub_cloud_trees,
                    ),
                    f,
                )
        ...
        if not os.path.exists(filename):
            ...
            for ep in tqdm(range(self.num_epochs)):
                for st in range(self.num_steps):
                    ...
                    points = np.array(
                        self.sub_cloud_trees[cloud_ind].data, copy=False
                    )  # sub_points
                    ...
                    query_inds = self.sub_cloud_trees[cloud_ind].query_radius(
                        pick_point,
                        r=self.in_radius,
                    ...
                    dists = np.sum(
                        np.square((points[query_inds] - pick_point).astype(np.float32)),
                        axis=1,
                    )
        ...
        filename = os.path.join(processed_dir, f"{split}_{voxel_size:.3f}_proj.pkl")
        if not os.path.exists(filename):
            proj_ind_list = []
    ...
    def __getitem__(self, idx):
        ...
        points = np.array(self.sub_cloud_trees[cloud_ind].data, copy=False)  # subpoints
        center_point = points[point_ind, :].reshape(1, -1)
        pick_point = center_point + noise.astype(center_point.dtype)
        # Indices of points in input region
        # the point index in the subsampled point tree.
        query_inds = self.sub_cloud_trees[cloud_ind].query_radius(
            pick_point, r=self.in_radius, return_distance=True, sort_results=True
        )[0][0]
        # Number collected
        cur_num_points = query_inds.shape[0]
        if self.num_points < cur_num_points:
        ...
        colors = self.sub_clouds_points_colors[cloud_ind][input_inds].astype(np.float32)
        labels = self.sub_clouds_points_labels[cloud_ind][input_inds].astype(np.int64)
        ...
        if "heights" not in data.keys():
            data["heights"] = torch.from_numpy(
                original_points[:, self.gravity_dim : self.gravity_dim + 1].astype(
                    np.float32
                )
            )
        return data
