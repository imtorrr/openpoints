...
class S3DIS(Dataset):
    ...
    def __init__(
        ...
        if split == "train":
            self.data_list = [
                item for item in data_list if "Area_{}".format(test_area) not in item
            ]
        else:
            self.data_list = [
        ...
        filename = os.path.join(
            processed_root,
            f"s3dis_{split}_area{test_area}_{voxel_size:.3f}_{str(voxel_max)}.pkl",
        )
        if presample and not os.path.exists(filename):
            ...
            for item in tqdm(
                self.data_list,
                desc=f"Loading S3DISFull {split} split on Test Area {test_area}",
            ):
