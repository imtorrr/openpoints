...
def vis_points(
    points, colors=None, labels=None, color_map="Paired", opacity=1.0, point_size=5.0
):
...
def vis_multi_points(
    ...
    for i in range(n_clouds):
        plotter.subplot(0, i)
        if len(points[i].shape) == 3:
            points[i] = points[i][0]
        if colors[i] is not None and len(colors[i].shape) == 3:
            colors[i] = colors[i][0]
...
def vis_neighbors(
    ...
    plotter = pv.Plotter()
    plotter.add_points(
        points, opacity=0.5, point_size=point_size, render_points_as_spheres=True
    )
    plotter.add_points(
        neighbor_points[point_index, :, :],
        point_size=neighor_point_size,
        color=neighbor_colors,
        render_points_as_spheres=True,
    )
    plotter.add_points(
        points[point_index],
        point_size=neighor_point_size * 2,
        color="green",
        render_points_as_spheres=True,
    )
    plotter.show()
