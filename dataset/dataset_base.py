...
class DatasetBase(dataset.Dataset):
    def __init__(
        self, dataset_name, split, cache_dir=None, load_cache_if_exists=True, **kwargs
    ):
    ...
    def cache_load_and_save(self, base_path, op, verbose):
        ...
        if op == "load":
            ...
            if (
                ...
                and hasattr(self, "_record_tokens")
                and hasattr(self, "_records")
            ):
                return
    ...
    def cache(self, path=None, verbose=1, must_exist=False):
        ...
        try:
            if verbose:
                print(f"Trying to load {self.split} cache from disk...", flush=True)
