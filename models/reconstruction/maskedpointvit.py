...
class MaskedPointViT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.decoder_pos_embed = nn.Sequential(
            nn.Linear(3, 128), nn.GELU(), nn.Linear(128, decoder_embed_dim)
        )
        ...
        self.decoder_pred = nn.Linear(
            decoder_embed_dim, embed_args.group_size * 3, bias=True
        )  # decoder to patch
    ...
    def initialize_weights(self):
        ...
        torch.nn.init.normal_(self.encoder.cls_pos, std=0.02)
        torch.nn.init.normal_(self.decoder_cls_pos, std=0.02)
    ...
    def random_masking(x, pos_embed, mask_ratio):
        ...
        pos_embed_masked = torch.gather(
            pos_embed, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D)
        )
    ...
    def forward_encoder(self, center_xyz, features, mask_ratio):
        ...
        features, pos_embed, mask, ids_restore, idx_keep = self.random_masking(
            features, pos_embed, mask_ratio
        )
        ...
        pos_embed = torch.cat(
            (self.encoder.cls_pos.expand(features.shape[0], -1, -1), pos_embed), dim=1
        )
    ...
    def forward_decoder(self, features, center_xyz, ids_restore):
        ...
        decoder_pos_embed = torch.cat(
            (
                self.decoder_cls_pos.expand(B, -1, -1),
                self.decoder_pos_embed(center_xyz),
            ),
            dim=1,
        )
        ...
        mask_tokens = self.mask_token.repeat(
            B, ids_restore.shape[1] + 1 - features.shape[1], 1
        )
    ...
    def forward(self, xyz, features=None):
        center_xyz, features, grouped_xyz, grouped_features = self.encoder.patch_embed(
            xyz, features
        )
        latent, mask, ids_restore, idx_keep = self.forward_encoder(
            center_xyz, features, self.mask_ratio
        )
