...
class MaskedTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.decoder_pred = nn.Linear(
            decoder_embed_dim, group_size * 3, bias=True
        )  # decoder to patch
        # ------------------------------------------------------------------
    ...
    def forward(self, center_xyz, features, ids_restore):
        ...
        mask_tokens = self.mask_token.repeat(
            B, ids_restore.shape[1] + 1 - L, 1
        )  # +1, since features contains additional cls token.
...
class FoldingNet(nn.Module):
    ...
    def __init__(
        self, in_channels, emb_dims=1024, num_fine=1024, grid_size=2, **kwargs
    ):
        super().__init__()
        if kwargs:
            logging.warning(f"kwargs: {kwargs} are not used in {__class__.__name__}")
        ...
        self.register_buffer(
            "folding_seed",
            torch.cat([a, b], dim=0).reshape(1, 2, self.grid_size**2).transpose(1, 2),
        )
        self.model_init()
    ...
    def forward(self, xyz, x, **kwargs):
        ...
        seed = (
            self.folding_seed.unsqueeze(1)
            .expand(B, self.num_coarse, -1, -1)
            .reshape(B, self.num_fine, -1)
        )
...
class NodeShuffle(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.convs = nn.Sequential(
            GraphConv(
                in_channels, emb_dims, conv, norm_args=norm_args, act_args=act_args
            ),
    ...
    def forward(self, xyz, feature, **kwargs):
        ...
        new_xyz = self.proj(feature.squeeze(-1).transpose(1, 2)).view(
            B, N, -1, 3
        ) + xyz.unsqueeze(2).repeat(-1, -1, self.up_ratio, -1)
