...
class StratifiedEncoder(nn.Module):
        ...
        **kwargs,
        ...
        if stem_transformer:
            self.stem_layer = nn.ModuleList(
                [
                    KPConvSimpleBlock(
                        in_channels, channels[0], prev_grid_size, sigma=sigma
                    )
                ]
            )
            self.layer_start = 0
    ...
    def forward_seg_feat(
        self, xyz, feats=None, offset=None, batch=None, neighbor_idx=None
    ):
        if hasattr(xyz, "keys"):
            xyz, feats, offset, batch, neighbor_idx = (
                xyz["pos"],
                xyz["x"],
                xyz["o"],
                xyz.get("batch", None),
                xyz.get("neighbor_idx", None),
            )
...
class StratifiedDecoder(nn.Module):
    def __init__(self, channels, up_k, num_layers=4, **kwargs):
        super().__init__()
