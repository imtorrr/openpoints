...
class RevBackProp(torch.autograd.Function):
    @staticmethod
    @custom_fwd
    def forward(ctx, x, blocks, pos_embed=None, alpha=0.0, lambd=1.0):
...
class InvPointViT(nn.Module):
        ...
        if self.option != -1:
            ...
            if epoch < self.init_epoch:  # Inference
                ...
                if self.option == 0.0:
                    alpha = round(
                        min(
                            max(
                                -round(
                                    self.freq / (self.end_epoch - self.start_epoch), 2
                                )
                                * math.ceil(
                                    (epoch - self.start_epoch - 1 - self.freq // 2)
                                    / self.freq
                                )
                                + 1,
                                0,
                ...
                # self.option 1: linear alpha = (1/(A+1-N))*x + N/(N-A-1), lambd = 1 - alpha + 0.01
                    ...
                    alpha = round(
                        1 / (self.start_epoch + 1 - self.end_epoch) * epoch
                        + self.end_epoch / (self.end_epoch - self.start_epoch - 1),
                        2,
                    )
                ...
                # self.option 3: exponential
                    ...
                    base = math.e
                    a = 1 / (
                        pow(base, self.start_epoch + 1) - pow(base, self.end_epoch)
                    )
                    ...
                    lambd = round(
                        max(
                            min(
                                ...
                                * math.ceil((epoch - self.start_epoch - 1) / self.freq)
                                + self.init_lambd,
                                1.0,
                            ),
                            self.init_lambd,
    ...
    def _use_inv(self, x, pos_embed=None, alpha=0.0, lambd=1.0):
