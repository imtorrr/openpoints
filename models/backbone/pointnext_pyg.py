...
class SetAbstraction(nn.Module):
    ...
    def forward(self, pxb):
        ...
        if self.is_head:
            ...
            edge_index = self.grouper(
                p, new_p, b, new_b
            )  # TODO: this part is extremly slow!
            dp = torch.index_select(p, 0, edge_index[1]) - torch.index_select(
                p, 0, edge_index[0]
            )
            xj = torch.index_select(x, 0, edge_index[1])
...
class FeaturePropogation(nn.Module):
    ...
    def forward(self, pxb1, pxb2=None):
        ...
        if pxb2 is None:
            ...
            x = torch.cat(
                (x, self.linear2(x_global).unsqueeze(-1).expand(-1, -1, x.shape[-1])),
                dim=1,
            )
            x = self.linear1(x)
        else:
...
class PointNextEncoderPyG(nn.Module):
    r"""The Encoder for PointNext
    `"DeepGCNs: Can GCNs Go as Deep as CNNs?"
    <https://arxiv.org/abs/1904.03751>`_.
    .. note::
        For an example of using :obj:`PointNextEncoderPyG`, see
...
@MODELS.register_module()
class PointNextDecoderPyG(nn.Module):
