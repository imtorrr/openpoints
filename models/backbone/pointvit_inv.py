...
class RevBackProp(torch.autograd.Function):
    ...
    def forward(ctx, x, blocks, pos_embed=None, alpha=0.0, lambd=1.0, num_cached=0):
        ...
        if len(buffer_layers) == 0:
            all_tensors = [
                x.detach(),
                pos_embed.detach() if pos_embed is not None else pos_embed,
            ]
        else:
...
class InvFuncWrapper(nn.Module):
    ...
    def forward(self, x, pos_embed=None, alpha=0.0, lambd=1.0):
    ...
    def backward_pass(self, Y_1, Y_2, dY_1, dY_2, pos_embed=None, alpha=0.0, lambd=1.0):
        """
        equations:
...
class InvPointViT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.norm = create_norm(
            norm_args, self.embed_dim
        )  # Norm layer is extremely important here!
    ...
    def initialize_weights(self):
        ...
        if self.dist_token is not None:
            ...
            torch.nn.init.normal_(self.dist_pos, std=0.02)
        ...
        if self.option != -1:
            ...
            if epoch < self.init_epoch:  # Inference
                ...
                # Option 0: linear, interleave alpha and lambda
                    ...
                    alpha = round(
                        min(
                            max(
                                -round(self.freq / (end_epoch - start_epoch), 12)
                                * math.ceil(
                                    (epoch - start_epoch - 1 - self.freq // 2)
                                    / self.freq
                                )
                                + 1,
                                0,
                ...
                # self.option 1: linear alpha = (1/(A+1-N))*x + N/(N-A-1), lambd = 1 - alpha + 0.01
                    ...
                    alpha = round(
                        1 / (start_epoch + 1 - end_epoch) * epoch
                        + end_epoch / (end_epoch - start_epoch - 1),
                        2,
                    )
                # self.option 2: logarithmic
                    ...
                    alpha = round(max(min(a * math.log(epoch, base) + c, 1.0), 0), 2)
                # self.option 3: exponential
                elif self.option == 3:
                    base = math.e
                    a = 1 / (pow(base, start_epoch + 1) - pow(base, end_epoch))
                    c = -a * pow(base, end_epoch)
                    alpha = round(max(min(a * pow(base, epoch) + c, 1.0), 0), 2)
                # self.option 4: hand-crafted, increasing
                elif self.option == 4:
                    alpha_list = [1.0, 1.0, 0.8, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0.0]
            ...
            if epoch < self.init_epoch:  # Inference
                ...
                if self.option == 6:
                    lambd = -1 * round(
                        max(
                            min(
                                ...
                                * math.ceil((epoch - start_epoch - 1) / self.freq)
                                + self.init_lambd,
                                1.0,
                            ),
                            self.init_lambd,
                    ...
                    lambd = round(min(1 - alpha + 0.1, 1.0), 2)
            elif epoch >= end_epoch:
                if self.option == 6:
                    lambd = -1.0
                else:
    ...
    def lambd_alpha_options(
        self, option=0, current=-1, start=0, end=10, freq=10.0, freeze_ratio=0.6
    ):
        """
        ...
        if option == 0:
                ...
                max(
                    min(
                        (
                            freq
                            / (end - start)
                            * math.ceil((current - start - 1) / freq)
                            + lambd_min
                        ),
                        lambd_max,
                    ),
            ...
            alpha = round(
                min(
                    max(
                        -round(freq / (end - start), 12)
                        * math.ceil((current - start - 1 - freq // 2) / freq)
                        + 1,
                        alpha_min,
                    ),
                    alpha_max,
        ...
        #     alpha = 1.0
                ...
                lambd = round(
                    max(
                        min(
                            freq
                            / (end - end_half)
                            * math.ceil((current - end_half - 1) / freq)
                            + lambd_min,
                            lambd_max,
                        ),
                        lambd_min,
            ...
            if current < end_half:
                ...
                alpha = round(
                    min(
                        max(
                            -round(freq / (end - end_half), 12)
                            * math.ceil((current - end_half - 1 - freq // 2) / freq)
                            + 1,
                            alpha_min,
                        ),
                        alpha_max,
    ...
    def get_alpha_lambd(
        ...
        if option == -1:
            ...
            if current < start:  # Use original non-reversible for the initial epoch
                ...
                lambd, alpha = self.lambd_alpha_options(
                    option, current, start, end, freq, freeze_ratio
                )
        return alpha, lambd
    ...
    def forward(self, p, x=None, epoch=-1, step=-1):
        if hasattr(p, "keys"):
            p, x, epoch, step = (
                ...
                p["epoch"] if "epoch" in p.keys() else -1,
                p["iter"] if "iter" in p.keys() else -1,
            )
        if x is None:
        ...
        with torch.set_grad_enabled(epoch >= self.start_epoch):
            alpha, lambd = self.get_alpha_lambd(
                epoch,
                step,
                option=self.option,
                start_epoch=self.start_epoch,
            ...
            if self.add_pos_each_block:
                if use_inv:
                    x = self._use_inv(
                        x, pos_embed, alpha, lambd, num_cached=self.num_cached
                    )
                else:
    ...
    def _use_inv(self, x, pos_embed=None, alpha=0.0, lambd=1.0, num_cached=0):
...
class InvPointViTDecoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if progressive_input:
            skip_dim = [
                self.in_channels // 2**i for i in range(n_decoder_stages - 1, 0, -1)
            ]
        else:
    ...
    def forward(self, p, f):
        ...
        if self.global_feat is not None:
            ...
            global_feats = torch.cat(global_feats, dim=1).expand(
                -1, -1, f_out.shape[-1]
            )
        ...
        **kwargs,
        ...
        if self.cls_map == "curvenet":
            ...
            self.global_conv1 = nn.Sequential(
                create_convblock1d(
                    fp_channels[-2] * 2, 64, norm_args=None, act_args=act_args
                )
            )
    ...
    def forward(self, p, f, cls_label):
        ...
        f[i - 1] = self.decoder[i][1:](
            [
                p[i],
                self.decoder[i][0](
                    [p[i - 1], torch.cat([cls_one_hot, f[i - 1]], 1)], [p[i], f[i]]
                ),
            ]
        )[1]
