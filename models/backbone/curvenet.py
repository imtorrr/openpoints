...
class Walk(nn.Module):
    ...
    def forward(self, xyz, x, adj, cur):
        ...
        batch_offset = (
            torch.arange(0, bn, device=torch.device("cuda")).detach() * tot_points
        )

        # indices of neighbors for the starting points
        tmp_adj = (adj + batch_offset.view(-1, 1, 1)).view(
...
curve_config = {"default": [[100, 5], [100, 5], None, None, None]}
...
class CurveNet(nn.Module):
    def __init__(self, num_classes=50, category=16, k=32, setting="default", **kwargs):
        ...
        self.up_cic2 = CIC(
            ...
            radius=0.2,
            k=k,
            in_channels=128 + 64 + 64 + category + 3,
            output_channels=256,
            bottleneck_ratio=4,
        )
        self.up_cic1 = CIC(
