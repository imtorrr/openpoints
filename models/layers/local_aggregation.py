            ...
            if self.use_pooled_as_identity:
                ...
                if query_xyz.shape[1] != support_xyz.shape[1]:
                    features = torch.gather(
                        features,
                        -1,
                        query_idx.unsqueeze(1).expand(-1, features.shape[1], -1),
                    )
            elif dp.shape[2] == 1:
                neighbor_dim = (
                    2  # this means the current layer is a aggragation all lauyers
                )
...
class LocalAggregation(nn.Module):
    ...
    def forward(self, query_xyz, support_xyz, support_features, query_idx=None):
        ...
        Returns:
           output features of query points: [B, C_out, 3]
        """
        return self.SA_CONFIG_operator(
            query_xyz, support_xyz, support_features, query_idx
        )
