...
class KMeansEmbed(nn.Module):
    ...
    def forward(self, xyz, features=None):
        ...
        # p_j = xyz
        p_i = torch.gather(
            centroids, 1, labels.unsqueeze(-1).expand(-1, -1, 3)
        )  # B, N, 3
        ...
        pooled_feat = scatter(
            neighborhood_features, idx, dim=1, reduce="max"
        )  # B, K, C
        reapted_feat = torch.gather(
            pooled_feat, 1, idx.expand(-1, -1, pooled_feat.shape[-1])
        )
        neighborhood_features = torch.cat([reapted_feat, neighborhood_features], dim=-1)
        out_features = scatter(
            self.conv2(neighborhood_features), idx, dim=1, reduce="max"
        )
