...
class EdgeConv(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        max_value, _ = torch.max(
            self.nn(
                torch.cat([x.expand(-1, -1, -1, edge_index.shape[-1]), x_j - x], dim=1)
            ),
            -1,
            keepdim=True,
        )
...
_GCN_LAYER_DEFAULT = dict(mrconv=MRConv, edgeconv=EdgeConv, edge=EdgeConv)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert out_channels > in_channels, (
            "#out channels should be larger than #in channels"
        )
        self.body = DynConv(
            in_channels,
