...
class SubsampleGroup(nn.Module):
    ...
    def forward(self, p, x=None):
        ...
        if x is not None:
            B, C, N = x.shape[:3]
            center_x = torch.gather(x, 2, idx.unsqueeze(1).expand(-1, C, -1)).unsqueeze(
                -1
            )
...
class P3Embed(nn.Module):
    ...
    def forward(self, p, f=None):
        ...
        for convs in self.convs:
            ...
            center_f = torch.gather(
                cur_f, 2, idx.unsqueeze(1).expand(-1, cur_f.shape[1], -1)
            )
            ...
            fj = convs[0](fj)
            fj = torch.cat(
                [self.pool(fj).expand(-1, -1, -1, self.group_size), fj], dim=1
            )
