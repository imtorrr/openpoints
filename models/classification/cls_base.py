...
class BaseCls(nn.Module):
    def __init__(self, encoder_args=None, cls_args=None, criterion_args=None, **kwargs):
        super().__init__()
        ...
        if cls_args is not None:
            in_channels = (
                ...
                else cls_args.get("in_channels", None)
            )
        ...
        self.criterion = (
            build_criterion_from_cfg(criterion_args)
...
class DistillCls(BaseCls):
    ...
    def get_logits_loss(self, data, gt):
        ...
        return logits, self.criterion(
            data, [logits, dist_logits], gt.long(), self.dist_model
        )
...
class ClsHead(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        in_channels = (
            len(self.global_feat) * in_channels
        ...
        for i in range(len(mlps) - 2):
            heads.append(
                create_linearblock(
                    mlps[i], mlps[i + 1], norm_args=norm_args, act_args=act_args
                )
            )
            if dropout:
        ...
        if self.global_feat is not None:
            ...
            for preprocess in self.global_feat:
                if "max" in preprocess:
                    global_feats.append(
                        torch.max(end_points, dim=self.point_dim, keepdim=False)[0]
                    )
                elif preprocess in ["avg", "mean"]:
                    global_feats.append(
                        torch.mean(end_points, dim=self.point_dim, keepdim=False)
                    )
