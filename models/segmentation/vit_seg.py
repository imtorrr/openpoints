...
from ..backbone import PointViT, PointNet2Decoder
...
class PointVitSeg(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.decoder = PointNet2Decoder(
            conv_args, norm_args, act_args, skip_channel_list, fp_mlps
        )
        self.head = SceneSegHeadPointNet(
            num_classes=num_classes, in_channels=fp_mlps[0][0]
        )
    ...
    def forward(self, xyz, features):
        ...
        for npoints in self.num_points[:-1]:
            ...
            l_features.append(
                torch.gather(
                    features, -1, idx.unsqueeze(1).expand(-1, features.shape[1], -1)
                )
            )
