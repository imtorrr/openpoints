...
def get_num_layer_for_vit(var_name, num_max_layer):
    ...
    if any(
        key in var_name
        for key in {"cls_token", "mask_token", "cls_pos", "pos_embed", "patch_embed"}
    ):
        return 0
...
def get_parameter_groups(
    ...
    for name, param in model.named_parameters():
        ...
        if (
            ...
            or any(key in name for key in skip_list)
        ):
            group_name = "no_decay"
        ...
        if filter_by_modules_names is not None:
            ...
            if filter_exist:
                ...
                this_weight_decay = filter_by_modules_names[module_name].get(
                    "weight_decay", this_weight_decay
                )
                scale = (
                    filter_by_modules_names[module_name].get("lr_scale", 1.0) * scale
                )
